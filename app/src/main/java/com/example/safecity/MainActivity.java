                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()) {
                            for (long x = 1; x <= (long) task.getResult().get("no_of_incidents"); x++) {
                                ...
                                googleMap.addMarker(new MarkerOptions().position(new LatLng((double) task.getResult().get("LOCATION_LAT_" + x), (double) task.getResult().get("LOCATION_LONG_" + x))).title("Type: " + task.getResult().get("INCIDENT_TYPES_" + x)).snippet("Description: " + task.getResult().get("INCIDENT_DESC_" + x)).icon(null));
